...
class PluginAuchanassettrackerEquipment extends CommonDBTM
{
    ...
    /**
     * Native GLPI form chrome for bulk accessory receipt (same as showForm).
     */
    public function showBulkForm(array $options = []): bool
    {
        $this->initForm(-1, $options);
        $options['formtitle'] = $options['formtitle']
            ?? __('Bulk add accessories', 'auchanassettracker');
        $options['target'] = $options['target']
            ?? (plugin_auchanassettracker_web_dir() . '/front/equipment.bulk.php');
        ...
        echo "</td><td>" . __('Equipment type', 'auchanassettracker') . $req . "</td><td>";
        Dropdown::showFromArray('itemtype', $type_choices, [
            'value' => $default_type,
            'width' => '220px',
        ]);
        ...
        echo "</td><td>" . __('Location') . $req . "</td><td>";
        if ($scope !== null) {
            echo Dropdown::getDropdownName('glpi_locations', $scope);
            echo Html::hidden('locations_id', ['value' => $scope]);
            echo "<div class='form-text'>"
        ...
        // Native Add button posts as name=add; keep bulk_add for the handler.
        echo Html::hidden('bulk_add', ['value' => 1]);
        $this->showFormButtons($options);
        return true;
    }
